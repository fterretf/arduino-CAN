...
#define FLAG_RXM0                  0x0
#define FLAG_RXM1                  0x0
...
//The mask essentially determines which bits to apply the acceptance filters to. If any mask bit is set to
//a zero, that bit will automatically be accepted,regardless of the filter bit.
// mask filter  msg   
// 0    x       x       accept (default)
// 1    0       filter  accept
// 1    1       !filter reject
...
int MCP2515Class::filterN(FilterN filter)
{
  ...
  for (int n = 0; n < 2; n++) {
    // standard only
    uint8_t mask = filter.mask[n];
    mask &= 0x7ff;
    writeRegister(REG_RXBnCTRL(n), FLAG_RXM0);

    writeRegister(REG_RXMnSIDH(n), filter.mask[n] >> 3);
    writeRegister(REG_RXMnSIDL(n), filter.mask[n] << 5);
  ...
  // filter bits
  for (int n = 0; n < 6; n++) {
    uint16_t id = filter.id[n];//code wrong without usage of id
    id &= 0x7ff; 
    writeRegister(REG_RXFnSIDH(n), id >> 3);
    writeRegister(REG_RXFnSIDL(n), id << 5);
...
int MCP2515Class::filterExtendedN(FilterExtendedN filter)
{
  ...
  for (int n = 0; n < 2; n++) {
    ...
    mask &= 0x1FFFFFFF;   //29 bits of extended msg id 
    writeRegister(REG_RXBnCTRL(n), FLAG_RXM0);
    writeRegister(REG_RXBnCTRL(n), FLAG_RXM0);
  ...
  for (int n = 0; n < 6; n++) {
    uint32_t id = filter.id[n];//code wrong without usage of id
    id &= 0x1FFFFFFF;     
    writeRegister(REG_RXFnSIDH(n), id >> 21);
    writeRegister(REG_RXFnSIDL(n), (((id >> 18) & 0x03) << 5) | FLAG_EXIDE | ((id >> 16) & 0x03));
    writeRegister(REG_RXFnEID8(n), (id >> 8) & 0xff);
...
void MCP2515Class::setPins(int cs, int csX, int irq)
{
  _csPin = cs;
  _csPinX = csX;
  _intPin = irq;
...
{
  //CS 6 and 7 are command to demux to CAN1, S2LP and CC130 device. 
	if( _csPin==7 && _csPinX==6) { 
		PORT->Group[0].OUTCLR.reg = MUX_IOPIN_MASK; // Switch both MUX pins to 0 at the same time
	}else{
...
{
	if( _csPin==7 && _csPinX==6){
		PORT->Group[0].OUTSET.reg = MUX_IOPIN_MASK; // Switch both MUX pins to 1 at the same time
	} 
  else{
